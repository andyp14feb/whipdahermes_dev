...
class HeartbeatService:
    ...
    def process_heartbeat(self, payload: HeartbeatPayload) -> int:
        ...
        if self.engine is None:
            ...
            try:
                self.machine_registry.upsert_from_heartbeat(machine_id, now, len(payload.sessions))
            except APIError:
                raise
        ...
        with sqlite_write_lock(), SQLSession(self.engine) as db:
            try:
                self.machine_registry.upsert_from_heartbeat(machine_id, now, len(payload.sessions), db=db)
            except APIError:
                raise
